...
const useExistingAccount = Boolean(process.argv[2] && process.argv[3]);
const runPrimaryJourney = process.argv.includes('--primary-journey');
const runCalendarCredentialFailure = process.argv.includes('--calendar-credential-failure');
...
const request = async (path, init = {}) => {
  const response = await fetch(BASE + path, {
    ...init,
    redirect: 'manual',
    headers: { ...(init.body ? { 'content-type': 'application/json' } : {}), ...init.headers, cookie: cookies() },
  });
  save(response);
  if (!response.ok) throw new Error(`${init.method || 'GET'} ${path}: HTTP ${response.status}: ${(await response.text()).slice(0, 200)}`);
  return response;
};
...
if (runPrimaryJourney) {
  let accountId;
  let transactionId;
  try {
    await step('dashboard reachable', async () => {
      await request('/api/dashboard');
    });
    await step('account create and reload', async () => {
      const marker = Date.now().toString(36);
      const created = await request('/api/accounts', {
        method: 'POST',
        body: JSON.stringify({ name: `Acceptance ${marker}`, type: 'transaction', openingBalance: 100 }),
      });
      accountId = (await created.json()).id;
      const listed = await (await request('/api/accounts')).json();
      if (!listed.some((account) => account.id === accountId)) throw new Error('created account was not returned');
    });
    await step('transaction create, confirm, and reload', async () => {
      const created = await request('/api/transactions', {
        ...
        body: JSON.stringify({
          ...
          amount: -12.34,
          accountId,
          merchant: `Acceptance ${Date.now().toString(36)}`,
          category: 'Testing',
          tags: ['acceptance'],
        }),
      });
      ...
      const listed = (await (await request(`/api/transactions?accountId=${encodeURIComponent(accountId)}`)).json()).transactions;
      const transaction = listed.find((item) => item.id === transactionId);
      if (transaction?.status !== 'confirmed') throw new Error('confirmed transaction was not returned');
    });
  } finally {
    if (transactionId) await request(`/api/transactions/${transactionId}`, { method: 'DELETE' });
    if (accountId) await request(`/api/accounts/${accountId}`, { method: 'DELETE' });
  }
}
if (runCalendarCredentialFailure) {
  await step('calendar connection rejects missing grant', async () => {
    const response = await fetch(BASE + '/api/calendar/connections', {
      method: 'POST',
      redirect: 'manual',
    ...
    if (response.status !== 409 || body?.error?.details?.connectRequired !== true) {
      throw new Error(`expected reconnect-required 409, received ${response.status}`);
    }
  });
}
